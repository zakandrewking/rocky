# ⚠️ DEPRECATED -- do not edit, do not push. Kept as a reference implementation only.
#
# This was the "motion agent": a body that did nothing until told, taking drive/turn/stop commands
# over TCP on port 8765. It has been replaced by device/rocky_agent.py -- the autonomous behaviour
# loop, which is now the one and only robot agent. Only one payload can run on the board at a
# time, and having two things called an agent, only one of which was being developed, was a
# standing source of confusion in the app, the docs and the discovery sweep.
#
# It is kept because it is the only worked example of the newline-JSON command protocol on this
# hardware, and because STEPS.md's hardware gate was proved against it. If a commanded-motion body
# is ever wanted again, start here rather than from scratch -- but start by reading
# device/rocky_agent.py first, since the reason this one lost is that a robot which only moves when
# told is a worse robot.
#
# Excluded from `pnpm robot:check` for the same reason apps/desktop is out of `pnpm test`: a gate
# that is permanently red teaches people to ignore the gate. This file is frozen, so it cannot rot.
#
# ---------------------------------------------------------------------------------------------
