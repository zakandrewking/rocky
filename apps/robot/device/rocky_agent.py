...
def _perform_gesture(task, now):
    ...
    profiles = {
        "forward": (GESTURE_MOVE_RPM, -GESTURE_MOVE_RPM, GESTURE_FORWARD_MS),
        "fast_forward": (GESTURE_FAST_RPM, -GESTURE_FAST_RPM, GESTURE_FAST_MS),
        "backward": (-GESTURE_MOVE_RPM, GESTURE_MOVE_RPM, GESTURE_BACKWARD_MS),
        # The recovery schedule established positive same-sign as right and negative as left.
        "turn_left": (-TURN_RPM, -TURN_RPM, GESTURE_QUARTER_TURN_MS),
        "turn_right": (TURN_RPM, TURN_RPM, GESTURE_QUARTER_TURN_MS),
        "turn_around": (TURN_RPM, TURN_RPM, TURN_MS),
    }
    profile = profiles.get(gesture)
    if profile is not None:
        left, right, duration = profile
