...
│   │   ├── PersonalityCatalog.swift — generated character metadata + persisted selection
│   │   ├── PersonalitySelectorView.swift — character card selector
...
│   ├── Resources/
│   │   └── RealtimeSessionConfig.json — gitignored; every character's session, regenerated by
│   │                                     scripts/generate.sh from services/device-api
│   └── Tests/
...
regenerates `RealtimeSessionConfig.json` from `services/device-api/src/characters/` and
`session.ts`, so every selectable personality, voice, and tool list stays defined in exactly one
place. Nothing is typed on the phone; nothing is committed. The robot's own IP is auto-discovered
...
personality button in the top-right opens cards for every registered character; the selection is
kept across launches and supplies that character's generated prompt and voice to the next
Realtime conversation. Switching a paused conversation ends it first rather than changing voice
and memory mid-thought. A tappable status row below the orb expands into a detail area (mirroring
